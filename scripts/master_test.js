...
async function runMaster1000Audit() {
  ...
  try {
    ...
    console.log('adminLoginRes:', adminLoginRes.statusCode, adminLoginRes.data, adminLoginRes.setCookie);
    ...
    // Set departure to 6 hours from now so check-in window is open (< 24h)
    await pool.query('UPDATE flights SET departure_datetime = DATE_ADD(NOW(), INTERVAL 6 HOUR) WHERE flight_id = ?', [testFlight.flight_id]);
